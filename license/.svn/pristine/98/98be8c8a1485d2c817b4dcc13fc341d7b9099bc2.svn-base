...
class MemberCompanyController extends AbstractActionController
{
    ...
    /**
     * 公司信息表dbadapter
     * @return \Member\Model\CompanyTable
     */
    public function getCompanyTable()
    {
        if (!$this->companyTable){
            $sm = $this->getServiceLocator();
            $this->companyTable = $sm->get('Member\Model\MemberCompanyTable');
        }
    ...
    public function aclCityAction()
    {
        if ($this->getRequest()->isXmlHttpRequest()){
            ...
            $id = (int)$data->id;
            $str = '<select name="city" id="city" onchange="_getArea()" class="form-control">';
